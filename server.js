...
// Database connection (Aiven MySQL via pool)
// Set credentials in .env file for local dev, or as env vars on Render
const db = mysql.createPool({
    host: process.env.DB_HOST,
    user: process.env.DB_USER,
    password: process.env.DB_PASSWORD,
    database: process.env.DB_NAME,
    port: parseInt(process.env.DB_PORT) || 15440,
    ssl: {
...
db.getConnection((err, connection) => {
    ...
    connection.release();
    // Initialize database after connection is verified
    initializeDatabase();
});
